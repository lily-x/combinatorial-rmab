""" from https://github.com/khalil-research/Neur2SP/blob/main/nsp/model2mip/net2mip.py """
...
class Net2MIPPerScenario(object):
    ...
    def _add_scenario_to_mip(self, scenario):
        ...
        for k, (wt, b) in enumerate(zip(W, B)):
            output_size, input_size = wt.shape
            ...
            for j in range(output_size):
                ...
                # print('j val ', j, 'output', output_size)

                if k < len(W) - 1: # all layers before the last layer
                    # print(f'  {j} add to X a')
                    ...
                    # print(f'  {j} add to X b')
                    X.append(self.gp_model.addVar(vtype=gp.GRB.CONTINUOUS, lb=-gp.GRB.INFINITY, name=x_name))

                # XX_str = f'XX {len(XX[-1])}' if k > 0 else ''
                # print('  ', j, len(X), XX_str, len(S), len(Z), f' with output {output_size} in {input_size}')
                ...
                ####################################################################
                # rearranging state and action: we expect [input] = (action, state)
                ####################################################################
                _eq = 0
            ...
            # L NOTE: I had to take this back an indent
                ...
                for i in range(input_size):
                    ...
                    if k == 0:
                        ...
                        if i < n_var:
                            ...
                            _eq += wt[j][i] * self.scenario_embedding[i - n_var]
                    else:
                        _eq += wt[j][i] * XX[-1][i]
